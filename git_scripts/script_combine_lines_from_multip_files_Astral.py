#!/usr/bin/env python

#### this script takes files with a given extension (.tre for example) and combines them 
###  into a single output file.   below, provide the extension you want to search and the
##	 name of the out-file. sometimes you need to make an empty output file before hand.
#	 !script must be in folder you are searching!
########################################################################################

import fileinput
import glob					#finds all the pathnames matching a given pattern.
import os

os.chdir("/ufrc/soltis/jjantzen/Dissertation/Analysis/2020_analyses/Astral/Rooted_trees/")

file_list=glob.glob("*bs50.tre") 		#! input a search term that is in the filename of all your gene trees

OutFileName="/ufrc/soltis/jjantzen/Dissertation/Analysis/2020_analyses/Astral/rooted_gene_trees_bs50.tre"		# output file containing all gene trees

OutFile=open(OutFileName, 'w')

                
for line in fileinput.input(file_list):
    OutFile.write(line)
                                          